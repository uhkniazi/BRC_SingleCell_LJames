# File: 07.2_bam_files_qa.R
# Auth: umar.niazi@kcl.ac.uk
# DESC: quality checks on the bam before read removal
# Date: 08/05/2017
...
q = paste0('select Sample.id as sid, Sample.group1, Sample.group2, Sample.title, File.* from Sample, File
           where (Sample.idData = 12) AND (File.idSample = Sample.id AND File.type like "%original bam%")')
...
csFiles.sorted = paste(csFiles, '.sorted.gz.bam', sep='')
# sort and create indexes
sapply(seq_along(csFiles), function(x) sortBam(csFiles[x], csFiles.sorted[x]))
csFiles.sorted = paste(csFiles.sorted, '.bam', sep='')
bf = indexBam(csFiles.sorted)
...
lAllBams = lapply(csFiles.sorted, function(x){
  write.qa(x, cvSeqnames)
})
names(lAllBams) = dfSample$title
setwd(gcswd)
n = make.names(paste('CBamScaffold original louisa single cell rds'))
lAllBams$meta = dfSample
lAllBams$desc = paste('CBamScaffold object from louisa single cell in original form', date())
...
lAllBams$desc = NULL
lAllBams$meta = NULL
pdf(file='Results/bam.original.qa.pdf')
par(mfrow=c(2,2))
